...
void MultA(int* C, int* A, int* B, int n){
	...
	if(n == 2){
		...
		cilk_spawn MultA(C+(size/4), A, B+(size/4), n/2);
		cilk_spawn MultA(C+((3*size)/4), A+(size/2), B+(size/4), n/2);
		cilk_spawn MultA(C+(size/2), A+(size/2), B, n/2);
		cilk_sync;
		cilk_spawn MultA(C+(size/2), A+((3*size)/4), B+(size/2), n/2);
		cilk_spawn MultA(C+((3*size)/4), A+((3*size)/4), B+((3*size)/4), n/2);
		cilk_spawn MultA(C+(size/4), A+(size/4), B+((3*size)/4), n/2);
		cilk_spawn MultA(C, A+(size/4), B+(size/2), n/2);
		cilk_sync;
		return;
	}
}
...
int main(int argc, char* argv[]){
	...
	if(input < 2){
		cout << "input size must be larger than 1" << endl;
		return 0;
	}
